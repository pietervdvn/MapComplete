...
describe("ImageProviders", () => {
    it("should work on a variaty of inputs", async () => {
        ...
        Utils.injectJsonDownloadForTests("https://commons.wikimedia.org/w/api.php?action=query&list=categorymembers&format=json&&origin=*&cmtitle=Category%3AStatue%20of%20Jacob%20van%20Artevelde%20in%20Ghent",
            {
                "batchcomplete": "",
                "continue": {
                    "cmcontinue": "file|42454c4749554d2050484f544f4752415048532054414b454e204f4e20323032322d31312d31332c203230323220494e204748454e542c20535441545545204f46204a41434f422056414e204152544556454c444520494e204748454e542c204e4f56454d42455220323032322e4a5047|125708690",
                    "continue": "-||"
                },
                "query": {
                    "categorymembers": [{
        ...
        async function expects(urls: string | string[], tags: Record<string, string>, providerName: string = undefined) {
            tags.id = "test/" + i
            i++
            if (!Array.isArray(urls)) {
                urls = [urls]
            }
            const images = await AllImageProviders.LoadImagesFor(new ImmutableStore(tags)).AsPromise((imgs) => imgs !== undefined && imgs.length > 0)
            ...
            for (const url of urls) {
                if (imageSources.indexOf(url) < 0) {
                    expect.fail( "Image " + url + " not found.\n\tTry one of:"+imageSources.join("\n\t"))
                }
        ...
        await expects("https://commons.wikimedia.org/wiki/Special:FilePath/File%3A11-04%20Gand%20180%20monument%20Jakob%20Van%20Artevelde.jpg?width=500&height=400",
